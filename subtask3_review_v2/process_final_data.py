...
def main():
    ...
    parser.add_argument("--root_dir", type=str, default="./ICLR_2023")  # 
    parser.add_argument("--target_dir", type=str, default="./subtask3_review_final_v2")
    parser.add_argument("--text_data_path", type=str, default="./subtask3_review_v2/processed")
    ...
    for subfolder in tqdm(all_subfolders):
        ...
        # but remove the pdf file under the target_paper_dir
        paper_pdf_file = os.path.join(target_paper_dir, f"{paper_id}.pdf")
        if os.path.exists(paper_pdf_file):
            os.remove(paper_pdf_file)
        # also remove the useless mac file
        ds_store_file = os.path.join(target_paper_dir, ".DS_Store")
        if os.path.exists(ds_store_file):
            os.remove(ds_store_file)
        ds_store_file_2 = os.path.join(target_paper_dir, "._.DS_Store")
        if os.path.exists(ds_store_file_2):
            os.remove(ds_store_file_2)
