...
def main(args):
    ...
    with open(args.paper_ids_file, "r") as f:
        all_selected_paper_ids = f.readlines()
    all_selected_paper_ids = [paper_id.strip() for paper_id in all_selected_paper_ids]
    df = df[df["ID"].isin(all_selected_paper_ids)]
    print(f"==> selected {len(df)} papers")
    # print(df["weakness_filed"][2])
    print((df["weakness_filed"].index))
    # print(df["weakness_filed"].unique())
    # exit()
    ...
    for i in tqdm(df.index):
        ...
        id = df["ID"][i]
        url = df["URL"][i]
        key_words = df["keywords"][i]
        track = df["track"][i]
        accpetance = df["acceptance"][i]
        scores = df["scores"][i]
    ...
    target_file = f"{conf_name}_{len(df)}_{args.target_file}"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--paper_ids_file', type=str, default='selected_paper_ids.txt', help='the file to save the selected paper ids')
