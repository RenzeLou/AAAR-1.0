...
v2 version will seperate items in the experiment list, and feed it to the model to generate the corresponding explanation, one by one.
'''
import copy
import random
import re
...
def main():
    ...
    for eval_ins in tqdm(eval_data):
        ...
        # now, one-by-one forward the each item in the experiment list to the model separately
        if args.oracle:
            experiment_list = gt_experiment
        else:
            experiment_list = pred_experiment
        ...
        if experiment_list_len == 0:
            ...
            for i, exp_item in enumerate(experiment_list):
                ...
                pred_explanation_item, _ = openai_chat_completion(client, eval_dict_explanation, explanation_template, decoding_args, model_name=args.api_name)
                # attach "1.", "2." back to the beginning of the item
                pred_explanation_item = f"{i+1}. {pred_explanation_item}"
                pred_explanation.append(pred_explanation_item)
