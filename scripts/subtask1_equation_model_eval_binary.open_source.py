...
from prompt_templates import EquationRewrite_Difficult, EquationRewrite_Easy, Equation_eval_binary
...
def main():
    ...
    end_time = time.time()
    outputs = new_outputs
    assert len(outputs) == len(eval_data) * 4, f"len(outputs): {len(outputs)}, len(eval_data): {len(eval_data)}"
    
    print("===> calculating the performances ...")
    precision = precision_score(answer_list, outputs)
    recall = recall_score(answer_list, outputs)
    f1 = f1_score(answer_list, outputs)
    
    results_list = []
    for i, ins in tqdm(enumerate(eval_data)):
        answer = answer_list[i]
        answer = str(answer)
        pred = outputs[4*i : 4*(i+1)]
        lables = answer_list[4*i : 4*(i+1)]
        results_list.append({
    ...
    print("="*20)
    print(f"F1: {f1:.2f}; P: {precision:.2f}; R: {recall:.2f}")
