...
def main():
    ...
    #     if "larger than the maximum number of tokens that can be stored in KV cache" in str(e) and "The model's max seq len" in str(e):
    #         pattern = r'tokens that can be stored in KV cache \((\d+)\)'
    #         match = re.search(pattern, str(e))
    #         if match:
    #             number = int(match.group(1))
    #         else:
    #             # devide the number of tokens by 2
    #             number = int(args.max_word_len / 2)
    #         llm = LLM(model=args.api_name, tensor_parallel_size=args.gpu_num, max_model_len=number)
    #     else:
    #         raise e
    ...
    for i, eval_ins in tqdm(enumerate(eval_data)):
        ...
        if args.oracle:
            ...
            experiment_list_len = len(experiment_outputs_list[i])
        ...
        all_input_prompts_explanations.append(explanation_input_prompt)
    
    print("===> explanation list generation ...")
    explanation_outputs = llm.generate(all_input_prompts_explanations, sampling_params)  
    explanation_outputs_text = process_output(explanation_outputs)
    explanation_outputs_list = [process_text_to_list(out) for out in explanation_outputs_text]    
