'''
given one ground-truth experiment (P1) and the concatnation of the prediction experiments (P2), let LLM decide whether P2 include P1.
vice versa
'''
import base64
import copy
...
from prompt_templates import Exp_entailment_v2  
...
def main():
    ...
    if args.paper_selection_path != "none":
        ...
        with open("/data/rml6079/projects/scientific_doc/temp/subtask2_experiment_human_anno/all_paper_ids.txt", "r") as f:
            all_paper_ids = f.readlines()
            all_paper_ids = [x.strip() for x in all_paper_ids]  
        print(f"==> you select all {len(set(all_paper_ids))} papers (including those deprecated) to evaluate.")
    ...
    for paper_id in tqdm(papers_ids_to_predict):
        ...
        # skip if the model prediction file does not exist
        ...
        # each ground-truth exp with concat of all prediction exps
        pred_exps_concat = " ".join(pred_exps)
        gt_decision_list = []  # binary list, each element indicate whether the corresponding gt_exp is successfully predicted
        for gt_exp in gt_exps:
            prompt_dict = {
                    "EXP1": gt_exp,
                    "EXP2": pred_exps_concat
            }
            ...
            gt_decision_list.append(decision)
            
        # each prediction exp with concat of all gt exps
        gt_exps_concat = " ".join(gt_exps)
        pred_decision_list = []  # binary list, each element indicate whether the corresponding pred_exp is successfully match with any gt_exp
        for pred_exp in pred_exps:
            ...
            if decision is not None:
                ...
                decision = 0
                print("** get wrong response from the model, set it to 0 by default.")
            pred_decision_list.append(decision)
        
        
        # calculate the score
        gt_entail_score = np.mean(gt_decision_list)
        pred_entail_score = np.mean(pred_decision_list)
        novel_exps = [pred_exps[i] for i in range(len(pred_exps)) if pred_decision_list[i] == 0]
        
        # save the decision and score
        results = {
            "id": paper_id,
            "ground_truth": gt_exps,
            "prediction": pred_exps,
            "ground_truth_decision": gt_decision_list,
            "prediction_decision": pred_decision_list,
        ...
        paper_eval_complete_cnt += 1
    
    
    print("="*20)
    print(f"Results saved to: {save_dir}")
    print(f"{paper_eval_complete_cnt} papers have been evaluated during this run.")
