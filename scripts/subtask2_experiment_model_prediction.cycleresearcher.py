...
def main():
    ...
    api_name_save = "CycleResearcher-" + api_name_save
    ...
    for eval_ins in tqdm(eval_data):
        ...
        # Complete the paper with the research background
        generated_papers, generated_text = researcher.generate_paper(
            topic=input_text_cut,
            max_tokens=512,
            n=1  # Generate a single paper
        )

        # construct it as a enumerated list
        exps = process_text_to_list(generated_text)
        experiment_outputs_list.append(exps)
    ...
    assert len(eval_data) == len(experiment_outputs_list), "find the length of the experiment outputs is not equal to the eval data. eval data: {}, experiment outputs: {}".format(len(eval_data), len(experiment_outputs_list))
    for i, eval_ins in tqdm(enumerate(eval_data)):  
        paper_id = eval_ins["id"] 
        ...
        save_result_dict = {
            ...
            "predicton": {
                ...
                "Why do you suggest these experiments?": "None",
            },
